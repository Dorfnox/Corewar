...
/*   Updated: 2018/06/02 14:57:55 by bpierce          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		xor_(t_corewar *core, t_process *process)
{
	...
	xor_result = a ^ b;
	ft_printf("xor result: %.8x\n", xor_result);
	process->reg[process->args[2][0]][0] = (uint8_t)(xor_result >> 24);
	process->reg[process->args[2][0]][1] = (uint8_t)(xor_result >> 16);
	process->reg[process->args[2][0]][2] = (uint8_t)(xor_result >> 8);
	process->reg[process->args[2][0]][3] = (uint8_t)(xor_result >> 0);
}

uint32_t	get_xor_args(
	t_corewar *core,
	t_process *process,
	uint16_t index,
	uint8_t arg_num)
